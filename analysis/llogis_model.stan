...
functions {
  ...
  vector log_h (vector t, real shape, real scale) {
    ...
    for (i in 1:num_elements(t)) {
      v_log_h[i] = log(shape)-log(scale)+(shape-1)*(log(t[i])-log(scale))-log(1+pow((t[i]/scale),shape));
    }
    return v_log_h;
  }
  ...
  vector log_S (vector t, real shape, real scale) {
    ...
    for (i in 1:num_elements(t)) {
      v_log_S[i] = -log(1+pow((t[i]/scale),shape));
    }
    return v_log_S;
  }
  
  // Defines the sampling distribution
  real surv_loglogistic_lpdf (vector t, vector d, vector LL, real shape, real scale) {
...
model {
  // Likelihood
  target += surv_loglogistic_lpdf(t | d, LL, shape, scale);
